...
export interface NOPTrade {
	id: string
	tradeId: string
	lockedAt: string // Must include the date so we can group them
	action: 'BUY' | 'SELL'
	pair: string
	amount: number
	rate: number
...
export interface OpeningPosition {
	pair: string;
	carriedOverBuyAmount: number;
	carriedOverBuyValue: number;
	carriedOverSellAmount: number;
	carriedOverSellValue: number;
}
...
export interface PairMetrics {
	...
	base: string
	quote: string
	openBuy: ActionSummary;
	openSell: ActionSummary;
	availability: {
		amountAvailableForSale: number;
		profit: number;
	}
	exposureAndRisk: ExposureRisk
}
...
export interface NOPCalculationResult {
	totalTrades: number
	dailySummary: DailySummary[] // A flat timeline for charting
	multiDayMetrics: Record<string, Record<string, PairMetrics>> // Record<Date, Record<Pair, Metrics>>
}

// Simulated mathematical engine abstracted from your main codebase
export function calculateNOPMetrics(
	trades: NOPTrade[], 
	riskLimits: RiskLimit[] = [],
	initialOpeningPositions: OpeningPosition[] = []
): NOPCalculationResult {
	
	// 1. Group all trades by Date, then by Pair
	const tradesByDate = new Map<string, Map<string, NOPTrade[]>>();
	...
	for (const trade of sortedTrades) {
		...
		if (!tradesByDate.has(dateKey)) {
			tradesByDate.set(dateKey, new Map<string, NOPTrade[]>());
		}
	...
	const multiDayMetrics: Record<string, Record<string, PairMetrics>> = {};
	...
	for (const date of sortedDates) {
		...
		const nextDayOpeningPositions: OpeningPosition[] = [];
		...
		for (const pair of Array.from(allPairs)) {
			...
			let positionType: 'LONG' | 'SHORT' | 'FLAT' = 'FLAT';
			if (netPosition > 0) positionType = 'LONG';
			if (netPosition < 0) positionType = 'SHORT';
