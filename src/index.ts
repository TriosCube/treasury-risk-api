...
import {
  ...
  RiskLimit,
  OpeningPosition,
} from "./engines/nop-engine";
import {
  generateLiquidityForecast,
  HistoricalData,
} from "./engines/alm-engine";
...
const swaggerDocument = {
  ...
  info: {
    ...
    version: "1.0.0",
    description:
      "Standalone API for Net Open Position (NOP) and Asset-Liability Management (ALM) calculations. Connect your trades and ledger data to get risk forecasts.",
  },
  servers: [
  ...
  paths: {
    "/nop/exposure": {
      post: {
        ...
        description:
          "Submit an array of raw trades and optional risk limits. Returns structured NOP metrics including explicit Open Buy, Open Sell, Available for Sale, and Profit.",
        requestBody: {
          required: true,
          content: {
            "application/json": {
              schema: {
                ...
                properties: {
                  ...
                  riskLimits: {
                    type: "array",
                    description:
                      "Optional limits to check utilization and breaches",
                    items: {
                      type: "object",
                      properties: {
                  ...
                  openingPositions: {
                    type: "array",
                    description:
                      "Optional carried-over positions from the previous period (e.g. yesterday closing balance)",
                    items: {
                      type: "object",
                      properties: {
                        pair: { type: "string", example: "USD/NGN" },
        ...
        responses: {
          "200": {
            description:
              "Successful NOP calculation returning explicit buy/sell, daily summary, and availability data",
          },
        },
      },
    },
    "/alm/forecast": {
      post: {
        ...
        description:
          "Submit historical ledger and trade aggregations. Returns predictive cash flow scenarios and alerts.",
        requestBody: {
          required: true,
          content: {
            "application/json": {
...
apiRouter.post("/nop/exposure", (req, res) => {
  ...
  const result = calculateNOPMetrics(
    trades,
    riskLimits || [],
    openingPositions || [],
  );
...
apiRouter.post("/alm/forecast", (req, res) => {
  const { data, forecastMonths } = req.body as {
    data: HistoricalData;
  ...
  const result = generateLiquidityForecast(data, forecastMonths || 12);
  res.json(result);
});
...
app.listen(PORT, () => {
  ...
  console.log(
    `📚 Swagger Docs (for SDK generation): http://localhost:${PORT}/docs`,
  );
});
